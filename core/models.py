...
class ItemCarrito(models.Model):
    ...
    # Cantidad con decimales (solo se usarán decimales en productos granel)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.producto.es_granel:
            # === PRODUCTO A GRANEL ===
            if self.cantidad < Decimal('0.001'):
                raise ValueError(f"La cantidad mínima para {self.producto.nombre} (granel) es 0.001 kg")
            
            # Opcional: puedes poner un mínimo más práctico como 0.1 kg
            # if self.cantidad < Decimal('0.100'):
            #     raise ValueError(f"La cantidad mínima recomendada para granel es 0.1 kg")
            
        else:
            # === PRODUCTO NORMAL (NO GRANEL) ===
            # Forzar cantidad entera
            self.cantidad = self.cantidad.quantize(Decimal('1'))
            
            if self.cantidad < Decimal('1'):
        ...
        # Descontar stock solo la primera vez (al crear el item)
        if not self.pk:
    ...
    def delete(self, *args, **kwargs):
        # Devolver stock al eliminar del carrito
        self.producto.stock += self.cantidad
        self.producto.save(update_fields=['stock'])
