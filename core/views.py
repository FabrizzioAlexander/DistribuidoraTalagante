...
def add_to_carrito(request, producto_id):
    ...
    if request.method == "POST":
        try:
            # Limpiar y convertir la cantidad (acepta tanto "1.5" como "1,5")
            cantidad_str = request.POST.get("cantidad", "1").strip().replace(',', '.')
            ...
            if not producto.es_granel:
                # PRODUCTO NORMAL → solo permite números enteros
                if not cantidad.is_integer():
                    messages.error(request, f"{producto.nombre} solo se puede comprar en cantidades enteras (1, 2, 3...)")
                    return redirect('catalogo')
                
                cantidad = cantidad.quantize(Decimal('1'))  # fuerza a entero

            else:
                # PRODUCTO A GRANEL → permite decimales
                if cantidad < Decimal('0.1'):
                    messages.warning(request, "La cantidad mínima recomendada para granel es 0.1 kg")
                    cantidad = Decimal('0.1')  # corrige automáticamente

        except Exception:
            messages.error(request, "Cantidad inválida. Usa números como 1 o 1.5")
            return redirect('catalogo')

    else:
        # Si alguien entra por GET (raro), usar 1 por defecto
        cantidad = Decimal('1')

    # ==================== PROCESAR CARRITO ====================
    try:
        # Obtener o crear carrito reciente (válido por 15 minutos)
        ...
        item.save()  # Aquí se valida stock y tipo de producto (granel o no)
        ...
        messages.success(
            request,
            f'<strong>{producto.nombre}</strong> × {cantidad_mostrar} {unidad} añadido al carrito',
            extra_tags='safe'
        )
        ...
        messages.error(request, "Ocurrió un error al añadir al carrito")
...
def gestion_estados(request):
    try:
        # === TODO TU CÓDIGO ACTUAL AQUÍ DENTRO ===
        busqueda = request.GET.get('q', '').strip()
        estado_filtro = request.GET.get('estado', '')
        ...
        if request.GET.get('export') == 'excel':
            # Aquí mantenemos el try interno que ya tenías
            try:
                from openpyxl import Workbook
                from openpyxl.styles import Font, Alignment, PatternFill
                from openpyxl.utils import get_column_letter
                # ... todo el código de Excel que ya tienes ...
                # (lo puedes dejar tal cual)
            except Exception as e:
                print(f"ERROR EXPORT EXCEL: {e}")
                messages.error(request, "Error al generar Excel.")
        ...
        # === ESTE ES EL DEBUG QUE NOS MUESTRA EL ERROR REAL ===
        import traceback
